...
int main(int argc, char **argv)
{
    ...
    // sort_order sort order based on input
    int sort_order;  // 1 for ascending, 2 for descending
    if(argc > 2) {
        cerr << "Usage: incorrect number of argument on the command line...\n”";
        exit(1);
    } else if ((argc == 1) || (strcmp(argv[1],"-a")==0) || (strcmp(argv[1],"-A")==0)){
        sort_order = 1;
    } else if((strcmp(argv[1],"-d")==0)||(strcmp(argv[1],"-D")==0)){
        sort_order = 2;
    } else{
        cerr << "Usage: Invalid entry for the command line option.\n";
        exit(1);
    }
